## 

This repository contains code for analyses and figures contained in this manuscript, which were most recently tested with R 4.1.0 on the last commit date.


Code was written by [Kirsten Nishino](https://github.com/klnishino) and [Adelaide Tovar](https://github.com/adelaidetovar), with specific contributions noted in each script.

This analysis is open source under an MIT license.
